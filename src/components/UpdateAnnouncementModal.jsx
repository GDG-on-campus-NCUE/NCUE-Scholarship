                        ...
                        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="relative bg-white shadow-2xl w-full flex-1 flex flex-col overflow-hidden ml-auto max-w-full cursor-default" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-white z-10 flex-shrink-0">
                                <div className="flex items-center gap-3">
                                    <button onClick={() => { if (!isSaving && window.confirm('確定關閉公告編輯模組？未儲存的資訊將遺失。')) onClose(); }} aria-label="返回上一步或關閉" className="text-gray-400 hover:text-gray-600 p-2 rounded-xl hover:bg-gray-50 transition-all"><ArrowLeft size={22} /></button>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-white hide-scrollbar relative">
                                ...
                                <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8 h-full animate-in fade-in duration-500">
                                    ...
                                    <div className={`lg:col-span-4 space-y-5 overflow-y-auto custom-scrollbar ${activeTab === 'basic' ? 'block' : 'hidden lg:block'}`}>
                                        <div className="space-y-1.5"><label htmlFor="update_title" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">標題 <span className="text-red-600 font-bold ml-1">*</span></label><input id="update_title" type="text" name="title" className={inputStyles} value={formData.title} onChange={(e) => setFormData(p => ({ ...p, title: e.target.value }))} /></div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1.5"><label htmlFor="update_internal_id" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">內部辨識名 (選填)</label><input id="update_internal_id" type="text" name="internal_id" className={inputStyles} value={formData.internal_id} onChange={(e) => setFormData(p => ({ ...p, internal_id: e.target.value }))} maxLength={20} /></div>
                                            <div className="space-y-1.5"><label htmlFor="update_category" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">分類 <span className="text-red-600 font-bold ml-1">*</span></label>
