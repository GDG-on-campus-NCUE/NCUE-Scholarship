                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex flex-col cursor-pointer" onClick={() => !isLoading && onClose()}>
                        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="relative bg-white shadow-2xl w-full flex-1 flex flex-col overflow-hidden ml-auto max-w-full cursor-default" onClick={(e) => e.stopPropagation()}>
                            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center bg-white z-10 flex-shrink-0">
                                <div className="flex items-center gap-3">
                                    <button onClick={() => { if (!isLoading && window.confirm('確定關閉公告編輯模組？未儲存的資訊將遺失。')) onClose(); }} aria-label="返回上一步或關閉" className="text-gray-400 hover:text-gray-600 p-2 rounded-xl hover:bg-gray-50 transition-all"><ArrowLeft size={22} /></button>
                                    <h2 className="text-lg font-bold text-gray-800 tracking-tight">新增公告</h2>
                                </div>
                                <button onClick={onClose} aria-label="關閉視窗" className="hidden sm:block text-gray-400 hover:text-gray-600 p-2"><X size={20} /></button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 sm:p-8 bg-white hide-scrollbar">
                                ...
                                {currentStep === 2 && (
                                    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-8 h-full animate-in fade-in">
                                        <div className="lg:hidden flex bg-gray-50/80 rounded-xl p-1.5 gap-1.5 border border-gray-100 flex-shrink-0" role="tablist">
                                            <button onClick={() => setActiveTab('basic')} role="tab" aria-selected={activeTab === 'basic'} className={"flex-1 py-2.5 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all " + (activeTab === 'basic' ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-black/5' : 'text-gray-500')}><Info size={14} /> 資訊</button>
                                            <button onClick={() => setActiveTab('content')} role="tab" aria-selected={activeTab === 'content'} className={"flex-1 py-2.5 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all " + (activeTab === 'content' ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-black/5' : 'text-gray-500')}><FileText size={14} /> 內文</button>
                                            <button onClick={() => setActiveTab('attachments')} role="tab" aria-selected={activeTab === 'attachments'} className={"flex-1 py-2.5 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all " + (activeTab === 'attachments' ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-black/5' : 'text-gray-500')}><Paperclip size={14} /> 附件</button>
                                        </div>
                                        <div className={"lg:col-span-4 space-y-5 overflow-y-auto custom-scrollbar " + (activeTab === 'basic' ? 'block' : 'hidden lg:block')}>
                                            <div className="space-y-1.5"><label htmlFor="title" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">標題 <span className="text-red-600 font-bold ml-1">*</span></label><input id="title" type="text" name="title" className={inputStyles} value={formData.title} onChange={handleChange} placeholder="公告標題..." /></div>
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1.5"><label htmlFor="internal_id" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">內部辨識名 (選填)</label><input id="internal_id" type="text" name="internal_id" className={inputStyles} value={formData.internal_id} onChange={handleChange} maxLength={20} placeholder="選填內部代碼" /></div>
                                                <div className="space-y-1.5"><label htmlFor="category" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">分類 <span className="text-red-600 font-bold ml-1">*</span></label>
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1.5"><label htmlFor="start_date" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">開始日期</label><input id="start_date" type="date" name="application_start_date" className={inputStyles} value={formData.application_start_date} onChange={handleChange} /></div>
                                                <div className="space-y-1.5"><label htmlFor="end_date" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">結束日期 <span className="text-red-600 font-bold ml-1">*</span></label><input id="end_date" type="date" name="application_end_date" className={inputStyles} value={formData.application_end_date} onChange={handleChange} /></div>
                                            </div>
                                            <div className="space-y-1.5"><label htmlFor="submission_method" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">送件方式</label><input id="submission_method" type="text" name="submission_method" className={inputStyles} value={formData.submission_method} onChange={handleChange} /></div>
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1.5"><label htmlFor="limitations" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">兼領限制</label><select id="limitations" name="application_limitations" className={inputStyles} value={formData.application_limitations} onChange={handleChange}><option value="">未指定</option><option value="Y">可兼領</option><option value="N">不可兼領</option></select></div>
                                                <div className="space-y-1.5"><label htmlFor="is_active" className="text-xs font-bold text-gray-500 uppercase tracking-wider ml-1">狀態</label><select id="is_active" name="is_active" className={inputStyles} value={formData.is_active} onChange={e => setFormData(prev => ({ ...prev, is_active: e.target.value === 'true' }))}><option value={true}>上架</option><option value={false}>下架</option></select></div>
                                            </div>
                                        </div>
